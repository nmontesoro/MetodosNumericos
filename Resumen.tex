...
\section{Solución de ecuaciones lineales}
...
Si tomamos $X^{(0)} = \begin{bmatrix}0\\0\\0\end{bmatrix}$ y queremos que 
$||X^{(k)} - X^{(k-1)}||<0.001$:
...
Como $k = 16$ es el primer entero positivo para el cual se cumple la condición 
impuesta, $X\approx X^{(16)}$ es solución al problema.
